...
exports.registro_lista = asyncHandler(async (req, res, next) => {
    await Registro.sync();
    res.render('registro/listagem', { registro: await Registro.findAll() });
});
...
exports.registro_editando = asyncHandler(async (req, res, next) => {
    ...
    if (registro) {
        res.render('registro/edicao', { registro: registro.dataValues });
    } else {
        res.render('registro/listagem');
    }
});
...
exports.registro_inserir = asyncHandler(async (req, res, next) => {
    ...
    try {
        if (qtd_horas && data && idFuncionario && idObra) {
            ...
            res.redirect('/registro/listagem');
        }
    } catch (error) {
